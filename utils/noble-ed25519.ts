...
// Bob Singor 12/24/2020 - removed async
...
function sha512ToNumberLE(...args: Uint8Array[]): bigint {
  const messageArray = concatBytes(...args);
  const hash = utils.sha512(messageArray);
...
export function getPublicKey(privateKey: bigint | number): Uint8Array;
export function getPublicKey(privateKey: PrivKey) {
  const privBytes = utils.sha512(ensurePrivInputBytes(privateKey));
...
export function sign(hash: Uint8Array, privateKey: Hex): Uint8Array;
export function sign(hash: string, privateKey: Hex): string;
export function sign(hash: Hex, privateKey: Hex) {
  const privBytes = utils.sha512(ensurePrivInputBytes(privateKey));
  const p = encodePrivate(privBytes);
  const P = Point.BASE.multiply(p);
  const msg = ensureBytes(hash);
  const r = sha512ToNumberLE(keyPrefix(privBytes), msg);
  const R = Point.BASE.multiply(r);
  const h = sha512ToNumberLE(R.toRawBytes(), P.toRawBytes(), msg);
  const S = mod(r + h * p, CURVE.n);
  const sig = new SignResult(R, S);
  return typeof hash === 'string' ? sig.toHex() : sig.toRawBytes();
}

export function verify(signature: Signature, hash: Hex, publicKey: PubKey) {
  ...
  const h = sha512ToNumberLE(signature.r.toRawBytes(), publicKey.toRawBytes(), hash);
